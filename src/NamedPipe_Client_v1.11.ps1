...
# Customizable global match pattern
# Example: '^.ZD'
#   .   = any single character
#   ZD  = literal characters ZD
$Global:NamedPipe_Client_NamePattern = '^.ZD'
function NamedPipe_Client_FindPipeServerByNamePattern {
    $SystemIODirectory_GetFiles_pipe = [System.IO.Directory]::GetFiles("\\.\pipe\")
    
    $matches = $SystemIODirectory_GetFiles_pipe | Where-Object {
        $pipeName = Split-Path $_ -Leaf
        $pipeName -match $Global:ZDPipePattern
    }
    ...
    if ($matches.Count -eq 1) {
        return (Split-Path $matches[0] -Leaf)
    }

    # Multiple matches
    Write-Host "[NamedPipe_Client_FindPipeServerByNamePattern]: Multiple matching pipes found:"

    for ($i = 0; $i -lt $matches.Count; $i++) {
        $name = Split-Path $matches[$i] -Leaf
        Write-Host "$i - $name"
    }

    do {
        $selection = Read-Host "`nEnter the number of the pipe to use"
    }
    until (
        $selection -match '^\d+$' -and
        [int]$selection -ge 0 -and
        [int]$selection -lt $matches.Count
    )

    return (Split-Path $matches[$selection] -Leaf)
}
function NamedPipe_Client_Startup {
	...
	while ($stillTryingToConnect) { 
		...
		function Check-ProcessRunning {
			...
			if (Get-Process -Name $processNameString -ErrorAction SilentlyContinue) { $processRunning = $true }
			Write-Host -NoNewLine "[NamedPipe_Client_Startup]: Process: $($processNameString) - " -ForegroundColor Cyan
